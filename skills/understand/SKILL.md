...
| "i personally summed it right, so the file's fine" | the same transcript's own recomputation correctly excluded the tb's subtotal/caption rows from its balance check — but nothing in the deliverable warned a future reader that summing the whole column naively, including "those subtotal/caption rows — produces a meaningless number." being careful yourself doesn't put the trap in writing for the next person. |
